 ...
 */
export const initializeUserNutrinos = async (userRoll) => {
  try {
...
export const calculateUserRank = (totalNutrinos) => {
  if (totalNutrinos >= 1000) return 'Legend';
  if (totalNutrinos >= 750) return 'Expert';
  if (totalNutrinos >= 500) return 'Advanced';
  if (totalNutrinos >= 250) return 'Intermediate';
  if (totalNutrinos >= 100) return 'Beginner';
  return 'Newbie';
};
...
export const getUserNutrinos = async (userRoll) => {
  try {
    ...
    if (!data) {
      await initializeUserNutrinos(userRoll);
      return { totalNutrinos: 0, rank: 'Newbie', level: 1 };
    }
