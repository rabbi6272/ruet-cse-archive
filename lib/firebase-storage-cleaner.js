...
class FirebaseStorageCleaner {
  ...
  cleanExistingFirebaseData() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      const valueMatches = firebasePatterns.some(pattern => pattern.test(value));
  ...
  interceptFirebaseStorage() {
    ...
    localStorage.setItem = (key, value) => {
      ...
      const isFirebaseData = firebasePatterns.some(pattern => 
        pattern.test(key) || pattern.test(value)
      );
      
      if (isFirebaseData) {
        // Obfuscate the key and value
        const obfuscatedKey = this.obfuscator.advancedHash(key, 'storage_key_salt');
        const obfuscatedValue = this.obfuscator.xorEncrypt(value, 'storage_value_key');
    ...
    localStorage.getItem = (key) => {
      ...
      if (obfuscatedValue) {
        const deobfuscatedValue = this.obfuscator.xorDecrypt(obfuscatedValue, 'storage_value_key');
  ...
  preventConsoleLeakage() {
    ...
    const obfuscateFirebaseInMessage = (message) => {
      if (typeof message === 'string') {
        return message
          .replace(/https:\/\/[\w-]+-default-rtdb\.firebaseio\.com/g, '[FIREBASE_DB_OBFUSCATED]')
          .replace(/[\w-]+\.firebaseapp\.com/g, '[FIREBASE_APP_OBFUSCATED]')
          .replace(/last-197cd/g, '[PROJECT_ID_OBFUSCATED]')
          .replace(/s-usc1a-nss-[\d-]+\.firebaseio\.com/g, '[FIREBASE_HOST_OBFUSCATED]');
      }
      return message;
    };
