...
export const isAuthorizedReviewer = (user) => {
  ...
  console.log("Checking authorization for:", { userName, userRoll, userRole });
  ...
  const isCodeReviewer = codeReviewers.some(reviewer => {
    ...
    if (reviewer.roll && userRoll === reviewer.roll) {
      console.log("Matched by roll:", reviewer.roll);
      return true;
    }
    ...
    if (userName === reviewerName || 
      ...
      console.log("Matched by name:", reviewer.name);
      return true;
    }
    ...
    const namePartsMatch = userNameParts.some(userPart => 
      reviewerNameParts.some(reviewerPart => 
        userPart === reviewerPart && userPart.length > 2
      )
    );
    
    if (namePartsMatch) {
      console.log("Matched by name parts:", reviewer.name);
      return true;
    }
  ...
  const hasRoleAccess = roleBasedAccess.some(role => 
    userRole.includes(role) || userRoll === role
  );
  
  if (hasRoleAccess) {
    console.log("Matched by role access");
  }
