...
export const sendGlobalNotification = async (notificationData) => {
  try {
    debugLog("Starting to send global notification", {
      type: notificationData.type,
    });

    // Get all student rolls from the JSON file
    const allStudentRolls = users.map((student) => student.roll);
    ...
    debugLog(
      `Global notification completed: ${successCount}/${allStudentRolls.length} successful`
    );

    return {
      success: true,
      count: successCount,
      total: allStudentRolls.length,
      results,
    };
  } catch (error) {
...
export const removeGlobalNotification = async (notificationType) => {
  try {
    ...
    console.log(
      `Global notifications of type '${notificationType}' removed from all users`
    );
...
export const hasAssemblyNotification = async () => {
  return new Promise((resolve) => {
    ...
    const sampleUsers = users.slice(0, 3).map((student) => student.roll);
    let checkCount = 0;
    ...
    const checkUser = (userRoll) => {
      ...
      onValue(
        ...
        (snapshot) => {
          checkCount++;
          const notifications = snapshot.val() || {};
          const hasAssembly = Object.values(notifications).some(
            (notification) =>
          ...
          if (checkCount === sampleUsers.length) {
            debugLog(
              `Assembly notification check complete. Found: ${foundAssembly}`
            );
...
export const sendAvengersAssemblyNotification = async (doubtData = null) => {
  try {
    debugLog(
      "🦸‍♂️ Starting Avengers Assembly notification process",
      doubtData
        ? {
    ...
    if (doubtData && doubtData.userDetails) {
      ...
      debugLog("📝 Personalized message created", {
        askerName,
    ...
    if (result.success && result.count > 0) {
      ...
      // Log for UI feedback
      console.log(
        "🦸‍♂️ AVENGERS ASSEMBLY ACTIVATED! All users have been notified to help solve doubts!"
      );
    } else {
...
export const removeAssemblyNotificationIfResolved = async () => {
  try {
    ...
    if (!unsolvedExists) {
      ...
      console.log(
        "✅ MISSION COMPLETE! All doubts solved - Avengers Assembly notification removed!"
      );
      return {
        success: true,
        message: "Assembly notification removed - mission complete!",
      };
    }

    return {
      success: true,
      message: "Unsolved doubts still exist, keeping assembly notification",
    };
  } catch (error) {
  ...
  try {
    ...
    if (unsolvedExists && !notificationExists) {
      ...
      return {
        success: true,
        action: "sent",
        message: "Assembly notification sent - Avengers assembled!",
        result,
      };
      ...
      return {
        success: true,
        action: "maintained",
        message: "Assembly notification maintained - doubts still exist",
      };
    } else {
      ...
      return {
        success: true,
        action: "none",
        message: "No doubts and no notification - all good",
      };
    }
  } catch (error) {
