...
class CodeSnippet {
  ...
  async push(roll) {
    try {
      ...
      const snippetId = `snippet_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`;
      
      // Structure the data properly
      const snippetData = {
        id: snippetId,
        title: this.obj.title || 'Untitled',
        description: this.obj.description || '',
        codeSnippet: this.obj.codeSnippet || '',
        language: this.obj.language || 'text',
        difficulty: this.obj.difficulty || 'Not Applied',
        tags: this.obj.tags || [],
        rollNumber: roll,
        isPublic: this.obj.isPublic !== false, // Default to true
        likesCount: 0,
        copiesCount: 0,
        comments: [],
      ...
      // Save under user's roll number with snippet ID
      const userSnippetRef = ref(this.db, `codes/${roll}/${snippetId}`);
      await set(userSnippetRef, snippetData);
      
      // Also save in public codes for easy retrieval
      if (snippetData.isPublic) {
        const publicSnippetRef = ref(this.db, `public_codes/${snippetId}`);
        await set(publicSnippetRef, {
          ...snippetData,
          authorRoll: roll
        });
      }
      
      console.log(`Code snippet uploaded successfully with ID: ${snippetId}`);
      return snippetId;
    } catch (error) {
      console.error("Error uploading code snippet:", error);
      throw new Error(`Failed to upload code snippet: ${error.message}`);
    }
  }
  async read(roll) {
    try {
...
class CommentsManager {
  ...
  async read(roll, snippetId) {
    try {
      const dbRef = ref(this.db, "codes/" + roll + "/" + snippetId);
      const snapshot = await get(dbRef);
      if (snapshot.exists()) {
        return snapshot.val();
      } else {
  ...
  async update(commentId, updates) {
    try {
      const dbRef = ref(this.db, "comments/" + commentId);
      await update(dbRef, updates);
    } catch (error) {
      throw error;
    }
