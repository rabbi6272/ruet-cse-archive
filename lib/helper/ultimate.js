...
class CodeSnippet {
  ...
  async push(rollNumber) {
    try {
      ...
      return {
        message: `Codesnippet inserted successfully.`,
        id: snippetId,
        rollNumber,
      };
    } catch (error) {
  ...
  async update(snippetId, updates) {
    try {
      const collectionRef = collection(CodelibraryDB, COLLECTION);
      const snapshots = await getDocs(collectionRef);
      ...
      for (const snapshot of snapshots.docs) {
        ...
        const nextSnippets = data.snippets.map((item, index) =>
          index === snippetIndex
            ? {
                ...item,
                ...updates,
