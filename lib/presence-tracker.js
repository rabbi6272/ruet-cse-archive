...
import { ref, onValue, onDisconnect, serverTimestamp, set, remove, push } from 'firebase/database';
...
export class PresenceTracker {
  ...
  startTracking(userRoll, userName, currentPage = null) {
    ...
    if (!db) {
      console.log('Database not available for presence tracking');
      return;
    }
    ...
    try {
      ...
      console.error('Failed to initialize Firebase refs for presence tracking:', error);
      return;
    }
    ...
    onValue(this.connectionRef, (snapshot) => {
      if (snapshot.val() === true) {
        ...
        console.log('Firebase connected for user:', userRoll);
        this.setUserOnline();
      } else {
        // We're disconnected
        console.log('Firebase disconnected for user:', userRoll);
      }
    });
  ...
  setUserOnline() {
    ...
    const presenceData = {
      name: this.currentUser?.name || 'Unknown User',
      roll: this.currentUser?.roll || userRoll,
    ...
    const unsubscribe = onValue(presenceRef, (snapshot) => {
      ...
      const activeUsers = Object.values(presenceData)
        ...
        .map(user => ({
          ...
          isInGracePeriod: !user.isOnline && (now - user.timestamp) < PRESENCE_TIMEOUT
        }))
        .sort((a, b) => {
  ...
  cleanupStaleSessions() {
    ...
    onValue(sessionsRef, (snapshot) => {
      ...
      Object.entries(sessionsData).forEach(([roll, session]) => {
        ...
        if (typeof session.lastActivity === 'object' && session.lastActivity.seconds) {
        ...
        if ((now - lastActivityTime) > PRESENCE_TIMEOUT) {
          console.log(`Cleaning up stale session for user: ${roll} (inactive for ${Math.round((now - lastActivityTime) / 60000)} minutes)`);
