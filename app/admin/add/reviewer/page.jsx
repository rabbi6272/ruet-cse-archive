...
export default function AddReviewer() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(reviewersRef, (snapshot) => {
      ...
      const reviewersList = Object.entries(reviewersData).map(([id, reviewer]) => ({
        id,
        ...reviewer
      }));
  ...
  const validateForm = () => {
    ...
    const existingReviewer = existingReviewers.find(
      reviewer => reviewer.roll === formData.roll || reviewer.email === formData.email
    );
    if (existingReviewer) {
      setMessage({ 
        type: 'error', 
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-lg shadow-md p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            You are not authorized to add reviewers. Only specific administrators can access this functionality.
          </p>
          <button
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Add New Reviewer</h1>
              <p className="text-sm text-gray-600">Add a new code reviewer to the system</p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-6">Reviewer Information</h2>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label className="flex items-center">
                      ...
                      />
                      <span className="text-sm text-gray-700">Can review code submissions</span>
                    </label>
                    ...
                    <label className="flex items-center">
                      <input
                        ...
                        className="mr-3 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                      />
                      <span className="text-sm text-gray-700">Can moderate doubt posts</span>
                    </label>
                    ...
                    <label className="flex items-center">
                      <input
                        ...
                        className="mr-3 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                      />
                      <span className="text-sm text-gray-700">Can delete posts</span>
                    </label>
                    ...
                    <label className="flex items-center">
                      <input
                        ...
                        className="mr-3 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                      />
                      <span className="text-sm text-gray-700">Can ban users</span>
                    </label>
                  </div>
                </div>
                  ...
                  {existingReviewers.map((reviewer) => (
                    <div key={reviewer.id} className="border border-gray-200 rounded-lg p-3">
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-medium text-gray-900 text-sm">{reviewer.name}</h4>
                          <p className="text-xs text-gray-500">Roll: {reviewer.roll}</p>
                          <p className="text-xs text-gray-500">{reviewer.department} • Batch {reviewer.batch}</p>
                        </div>
