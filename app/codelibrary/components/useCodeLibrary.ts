 ...
 * each snippet object and are preserved as-is.
 */
function flattenDocs(snap: any): Snippet[] {
  const result: Snippet[] = [];

  snap.docs.forEach((snapDoc: any) => {
    const data = snapDoc.data();
    const snippetsArr: any[] = Array.isArray(data.snippets)
      ? data.snippets
      : [data]; // fallback: doc itself is the snippet

    snippetsArr.forEach((raw) => {
      if (!raw) return;
      const id: string = raw.id ?? snapDoc.id;
      result.push({
        ...raw,
        id,
        ...
        comments: raw.comments ?? [], // ← preserve embedded comments
      });
    });
  });
...
/** Merge two arrays by id, keeping newest-first order. */
...
/**
 * Read-modify-write a single snippet inside its roll document.
 * Uses getDoc (1 read) instead of getDocs (full collection scan).
 */
async function patchSnippetInFirestore(
  ...
  const nextSnippets = (Array.isArray(data.snippets) ? data.snippets : []).map(
    (item: any) => (item.id === snippetId ? updater(item) : item)
  );
...
export function useCodeLibrary(initialSnippets: Snippet[] = []) {
  ...
  const filteredSnippets = snippets.filter((s) => {
    ...
    if (languageFilter && s.language !== languageFilter.toLowerCase())
      return false;
    if (authorFilter && s.author?.toLowerCase() !== authorFilter.toLowerCase())
      return false;
    return true;
  });
  ...
  const toggleLike = async (id: string): Promise<void> => {
    ...
    const snippet = snippets.find((s) => s.id === id);
    if (!snippet || snippet.isLiked) return; // one-way like

    setAnimateLike((prev) => ({ ...prev, [id]: true }));
    setTimeout(
      () => setAnimateLike((prev) => ({ ...prev, [id]: false })),
      500
    );
