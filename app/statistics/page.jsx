...
import { getStudentName, createStudentMapping, formatName } from "@/lib/students-loader";
...
const StatisticsPage = () => {
  ...
  const processNutrinosData = (allUsersData) => {
    ...
    const users = Object.entries(allUsersData).map(([roll, data]) => {
      ...
      const displayName = data.name ? formatName(data.name) : 
                          studentMapping[roll] || 
                          getStudentName(roll);
    ...
    users.forEach(user => {
      ...
      user.history.forEach(entry => {
        ...
        monthlyStats[monthKey] = (monthlyStats[monthKey] || 0) + Math.abs(entry.nutrinos || 0);
      });
    });
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-6 text-lg text-gray-600 dark:text-gray-400 font-medium">Loading statistics...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-8 lg:mb-12">
            ...
            <p className="text-base sm:text-lg lg:text-xl text-gray-600 dark:text-gray-400 max-w-3xl mx-auto leading-relaxed">
              Discover the incredible contributions and achievements of our coding community
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-8 lg:mb-12">
            <div className="bg-white dark:bg-gray-800 rounded-lg lg:rounded-2xl p-4 lg:p-6 shadow-lg border border-gray-200 dark:border-gray-700 text-center">
              ...
              <div className="text-xl lg:text-3xl font-bold text-gray-900 dark:text-white">{totalUsers}</div>
              <div className="text-sm lg:text-base text-gray-600 dark:text-gray-400">Active Coders</div>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-lg lg:rounded-2xl p-4 lg:p-6 shadow-lg border border-gray-200 dark:border-gray-700 text-center">
              ...
              <div className="text-sm lg:text-base text-gray-600 dark:text-gray-400 mt-2">Total Earned</div>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-lg lg:rounded-2xl p-4 lg:p-6 shadow-lg border border-gray-200 dark:border-gray-700 text-center">
              <div className="text-2xl lg:text-3xl mb-2">📈</div>
              <div className="text-xl lg:text-3xl font-bold text-green-600 dark:text-green-400">{topUsersMonth.length}</div>
              <div className="text-sm lg:text-base text-gray-600 dark:text-gray-400">Active This Month</div>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-lg lg:rounded-2xl p-4 lg:p-6 shadow-lg border border-gray-200 dark:border-gray-700 text-center">
              <div className="text-2xl lg:text-3xl mb-2">🏆</div>
              <div className="text-xl lg:text-3xl font-bold text-yellow-600 dark:text-yellow-400">{topUsersAllTime.length}</div>
              <div className="text-sm lg:text-base text-gray-600 dark:text-gray-400">Top Contributors</div>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6 lg:gap-8 mb-8 lg:mb-12">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg lg:rounded-2xl p-6 lg:p-8 shadow-lg border border-gray-200 dark:border-gray-700">
              <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white mb-6 lg:mb-8 flex items-center">
                <span className="text-2xl sm:text-3xl lg:text-4xl mr-2 lg:mr-3">🏆</span>
                All-Time Avengers
              </h2>
              <div className="space-y-3 lg:space-y-4">
                {topUsersAllTime.map((user, index) => (
                  <div key={user.roll} className="group">
                    <div className={`relative overflow-hidden rounded-lg lg:rounded-xl p-3 lg:p-4 ${
                      ...
                      index === 1 ? 'bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800/20 dark:to-gray-700/20 border-gray-200 dark:border-gray-600' : 
                      index === 2 ? 'bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 border-orange-200 dark:border-orange-700' : 
                      'bg-gray-50 dark:bg-gray-700/50 border-gray-200 dark:border-gray-600'
                    } border transition-all duration-300 hover:shadow-md`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3 lg:space-x-4">
                          <div className={`w-8 h-8 lg:w-12 lg:h-12 rounded-full flex items-center justify-center text-white font-bold text-sm lg:text-lg ${
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="font-bold text-gray-900 dark:text-white text-sm sm:text-base lg:text-lg truncate">{user.name}</p>
                            <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                              Roll: {user.roll} • {getUserDisplayRole({ roll: user.roll })} • Level {user.level}
                            </p>
                          </div>
                        </div>
                        <div className="text-right flex-shrink-0">
                          ...
                          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1">Total Nutrinos</p>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-3 lg:space-y-4">
                {topUsersMonth.length > 0 ? (
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3 lg:space-x-4">
                            ...
                            <div className="min-w-0 flex-1">
                              <p className="font-bold text-gray-900 dark:text-white text-sm sm:text-base lg:text-lg truncate">{user.name}</p>
                              <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                                Roll: {user.roll} • {getUserDisplayRole({ roll: user.roll })}
                              </p>
                            </div>
                          </div>
                          <div className="text-right flex-shrink-0">
                  ...
                  <div className="text-center py-8 lg:py-12">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-base lg:text-lg">Be the first to make your mark this month!</p>
                  </div>
                )}
              </div>
            </div>
