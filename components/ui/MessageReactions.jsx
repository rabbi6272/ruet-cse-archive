...
const MessageReactions = ({
  ...
  const isEmojiPanelOpen = onShowEmojiPanelChange
    ? showEmojiPanel
    : internalShowEmojiPanel;
  const setShowEmojiPanelState = onShowEmojiPanelChange
    ? onShowEmojiPanelChange
    : setInternalShowEmojiPanel;
  ...
  useEffect(() => {
    ...
    console.log(
      "🎭 [MessageReactions] Setting up real-time listener for:",
      `${chatPath}/messages/${messageId}/reactions`
    );
  ...
  const handleReaction = async (emoji) => {
    ...
    console.log(
      "🔍 [WHATSAPP-REACT] String comparison:",
      `"${userCurrentReaction}" === "${emoji}"`,
      "=",
      userCurrentReaction === emoji
    );
    console.log(
      "🔍 [WHATSAPP-REACT] Type check:",
      typeof userCurrentReaction,
      typeof emoji
    );
    ...
    try {
      ...
      console.log(
        "🧹 [WHATSAPP-REACT] After removing user reactions:",
        updatedReactions
      );
      ...
      if (userCurrentReaction !== emoji) {
        ...
        console.log(
          `✅ [WHATSAPP-REACT] ${actionType}: ${
            userCurrentReaction || "none"
          } → ${emoji}`
        );
        ...
        console.log(
          `🗑️ [WHATSAPP-REACT] WITHDRAW: User clicked same emoji ${emoji} again`
        );
        console.log(
          `🗑️ [WHATSAPP-REACT] Current reaction: "${userCurrentReaction}", Clicked: "${emoji}"`
        );
        console.log(
          "🗑️ [WHATSAPP-REACT] User reaction should be removed (already done in STEP 1)"
        );
      ...
      console.log(
        "✅ [WHATSAPP-REACT] Successfully applied WhatsApp-style reaction behavior!"
      );
  ...
  const getTotalReactionsCount = () => {
    ...
    return displayReactions.reduce(
          ...
          >
            <div className="flex flex-wrap gap-1 sm:gap-2 justify-center max-w-full">
              {EMOJI_REACTIONS.map((reaction) => {
                ...
                const reactionCount = reactions[reaction.emoji]
                  ? Object.keys(reactions[reaction.emoji]).length
                  : 0;
                    ...
                    <button
                      ...
                      title={
                        isDisabled
                          ...
                          : `${reaction.name}${
                              isUserReaction
                                ? " (your current reaction - click to remove)"
                                : ""
                            }${
                              reactionCount > 0
            ...
            <div
              className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 text-center mt-2 sm:mt-3 px-1 sm:px-2 leading-tight"
              style={{ userSelect: "none", WebkitUserSelect: "none" }}
            >
              {userCurrentReaction
